		...
		return this.each(function(){
			...
			if(!$t.p.multiselect) {	
				...
				if (  ia === -1 ){
					...
					stat = true;
                    $("#jqg_"+$.jgrid.jqID($t.p.id)+"_"+$.jgrid.jqID($t.p.selrow)).attr("checked",stat); //参考旧版本jqgrid代码，修复多行选择bug
...
//				$("#jqg_"+$.jgrid.jqID($t.p.id)+"_"+$.jgrid.jqID($t.p.selrow))[$t.p.useProp ? 'prop': 'attr']("checked",stat);    //该行代码引起多行选择bug
				if(fid) {
					if(ia === -1) {
						$("#"+$.jgrid.jqID(selection), "#"+$.jgrid.jqID(fid)).addClass("ui-state-highlight");
					} else {
	...
	footerData : function(action,data, format) {
		...
		this.each(function(){
			...
			$(this.p.colModel).each(function(i){
//				nm = this.name;
                nm = this.index;    //修改从colModel的index中获取索引
				if(action == "set") {
					if( data[nm] !== undefined) {
