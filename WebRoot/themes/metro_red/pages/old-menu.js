...
MENU.subMenu = function (divName, menuId) {
    if (!document.getElementById("trg_" + menuId)) {
        ...
        rdcp.request("DS_USER_MENU", "parent_id="+menuId, function (resp) {
            if (resp != undefined && resp != null && resp.length) {
                for (var i = 0; i < resp.length; i++) {
                    ...
                    rdcp.request("DS_USER_MENU", "parent_id=" + resp[i]["id"],
                            function (resp3) {
...
MENU.openIFrame = function (id, name, url, canClose) {
    ...
    if (count < 0)//小于0为没有打开
    {
        ...
        $menu_li.attr("id", openmenulist);

        var $open_menu = $("<div>");
        $open_menu.attr("id","open_menu_"+id);
        $open_menu.addClass("projectbuttom");
        $open_menu.on("mouseover", function(){
            $(this).removeClass("projectbuttom")
            $(this).addClass("projectbuttom_hover");
        });$open_menu.on("mouseout", function(){
            $(this).addClass("projectbuttom")
            $(this).removeClass("projectbuttom_hover");
        });


        var $menuLink = $("<a>");
        $menuLink.css("width","100px");
        $menuLink.addClass("openlink");
        $menuLink.html(name);
        $menuLink.on("click", function(){
            MENU.showFrame(divid);
            return false;
        });

        var $closeLink = $("<a>");
        $closeLink.attr("href", "#");
        $closeLink.attr("target", frameid);
        $closeLink.addClass("close");
        $closeLink.on("click", function(){
            MENU.removeFrame(frameid);
            return false;
        });

        $open_menu.append($menuLink);
        if(canClose == undefined)
            $open_menu.append($closeLink);
        $menu_li.append($open_menu);


        var $htmlLi = $menu_li;  //创建DOM对象
