 ...
 * @param p
 *      mask:   true/false  是否进行遮罩
 *      filterError:    true/false  是否对系统的错误进行过滤
 */
rdcp.request = function (url, params, callback, p) {
    ...
    p["filterError"] = p["filterError"] == undefined ? true : p["filterError"];
    if (p["mask"])
    ...
    p.success = function (data, textStatus, jqXHR) {
        ...
        if (data.header != undefined && p["filterError"] && !rdcp.checkResponseCode(data.header))
            return;
 ...
 * @param passwd
 * @param captcha
 * @param onLogin 登录成功后的回调
 */
rdcp.login = function (name, passwd, captcha, onLogin) {
    rdcp.request("DS_USER_LOGIN", {validateCode: captcha, loginName: name, password: passwd},
            function (data) {
 ...
 * 检查会话状态
 * @param time 下次检查时间间隔
 * @param onLogout 检测到注销状态后进行的操作，比如跳到登录页面
 */
rdcp.checkSession = function (time,onLogout) {
    if (onLogout)rdcp.onLogout = onLogout;
    rdcp.request("!sys/security/~java/UpdateSession", {}, function (data) {
    	if(data.header==undefined)
    		return;
        if (data.header.code == "0") {
            //现在处于登录状态，过60秒再检查
            setTimeout("rdcp.checkSession("+time+");", time);
        } else if(data.header.code == "2003"){
            //什么事也不做
        }else {
            //已经注销，或者被踢出，显示信息，不再检测，并回调事件
            if ($.messager) {
                $.messager.alert('提示', '由于：' + data.header.message + '，请重新登录', 'warn', function () {
                    if (rdcp.onLogout)
                        rdcp.onLogout();
                });
            } else {
                alert('由于：' + data.header.message + '，请重新登录');
                if (rdcp.onLogout)
                    rdcp.onLogout();
            }
        }
