app.registerCtrl("ThreeGKaiHuCtrl", ["$scope", "$srhttp", "$rootScope",
	...
	function($scope, $srhttp, $filter, $rootScope) {
		...
		$scope.cardChange = function(type) {
			...
			if (type == 1) {
				...
				$scope.numType.value = "ChinaUnicom.Account.3G.BK";
				$scope.form.businessId = "ChinaUnicom.Account.3G.BK";
			} else {
				$(".com_area .right a:first-child").removeClass("choiceon_btn");
				$(".com_area .right a:last-child").addClass("choiceon_btn");
				$scope.numType.value = "ChinaUnicom.Account.3G.CK";
				$scope.form.businessId = "ChinaUnicom.Account.3G.CK";
			}
		...
		$scope.selectNumber = function(index) {
			...
			Local.saveStoreJson("numTypeParams3", $scope.numType);
			//锁卡
			$scope.lock();
		}
