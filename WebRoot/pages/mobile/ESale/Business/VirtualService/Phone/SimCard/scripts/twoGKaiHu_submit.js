...
app.registerCtrl("twoGKaiHuSubmitCtrl", ["$scope", "$srhttp", "$rootScope",
	function($scope, $srhttp, $rootScope) {
		...
		$scope.form = {
			...
			"package_name": $("#packageName .com_selectbtnon").text(), //!套餐名称,为空不用传值+
			"package_id": $("#packageName .com_selectbtnon").attr("packgeid"), //!套餐ID,为空不用传值+
		...
		$scope.numType = Local.getStoreJson("numTypeParams");
		//		
		//		$scope.form.package_id=Local.getStoreJson("packageID");
		//		$scope.form.package_name=Local.getStoreJson("packageName");
		$scope.packageRows = Local.getStoreJson("packageRows");
		$scope.packageNameChange = function(id) {
			var a = $("#packageId_" + id);
			if (a) {
				$(a).addClass("com_selectbtnon").parent().siblings().find("a").removeClass("com_selectbtnon");
				$scope.form.package_name = $(a).text();
				$scope.form.package_id = $(a).attr("packgeid");
			}
		}
		...
		$scope.submit = function() {
			...
			$scope.form.package_name = $("#packageName .com_selectbtnon").text();
			$scope.form.package_id = $("#packageName .com_selectbtnon").attr("packgeid");
		...
		$scope.getData = function() {
			...
			result['ext'] = {
				'ext': {
					...
					'BUSINESS': [{
						...
						'key': 'FIRST_MONTH_FEE',
						'val': $scope.form.FIRST_MONTH_FEE,
						'key_as': '首月支付'
					}, {
						'key': 'CardID',
			...
			if ($scope.numType.value == 'ChinaUnicom.Account.3G.CK') {
		...
		$scope.create_order = function(paramData) {
			$srhttp.request("!ESale/Mall/Order/~java/Order.create", {
					...
					'sureFee': $scope.form.FIRST_MONTH_FEE
				},
				function(data) {
					if (data.header.code == 0) {
						...
						Local.saveStoreJson("numTypeParams", "{}");
