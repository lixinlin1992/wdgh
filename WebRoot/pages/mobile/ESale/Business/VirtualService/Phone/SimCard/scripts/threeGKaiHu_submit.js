...
app.registerCtrl("ThreeGKaiHuSubmitCtrl", ["$scope", "$srhttp", "$rootScope",
	function($scope, $srhttp, $rootScope) {
		...
		//添加上一步跳转的链接
		app.when("/threeGKaiHu_package", {
			templateUrl: "ESale/Business/VirtualService/Phone/SimCard/pages/threeGKaiHu_package.html",
			controller: "ThreeGKaiHuPackageCtrl"
		});
		...
		$scope.getData = function() {
			...
			result['ext'] = {
				'ext': {
					...
					'BUSINESS': [{
						'key': 'package_name',
						'val': $scope.form.package_name+$scope.form.packageFee,
						'key_as': '套餐名称'
					}, {
						'key': 'package_id',
			...
			if ($scope.form.CARD_PIC || $scope.form.CARDNUMBER) {
		...
		$scope.create_order = function(paramData) {
			$srhttp.request("!ESale/Mall/Order/~java/Order.create", {
					...
					'businessId': $scope.form.businessId, //来自刚选号时，传过来的，比如：ChinaUnicom.Account.3G.BK
				...
				function(data) {
					if (data.header.code == 0) {
						...
						Local.saveStoreJson("numTypeParams3", "{}");
