...
function getActiTypeCode(a) {
    ...
    if (a.indexOf("存费送费") != -1) {
        ...
        if (a.indexOf("存费送业务") != -1) {

            return "ZSYW001"

        } else {

            if (a.indexOf("合约惠机B1") != -1) {

                return "HYHJ002"

            } else {

                if (a.indexOf("合约惠机B2") != -1) {

                    return "HYHJ003"

                } else {

                    if (a.indexOf("合约惠机C1") != -1) {

                        return "HYGJ002"

                    } else {

                        if (a.indexOf("合约惠机C2") != -1) {

                            return "HYGJ003"

                        } else {

                            if (a.indexOf("合约惠机C") != -1) {
...
app.registerCtrl("HandleContractCtrl4", ["$scope", "$srhttp", "$rootScope",
    function($scope, $srhttp, $rootScope) {
        $scope.TERMINAL_SN = ""; //终端串号
        $scope.TERMINAL_TYPE =""; //终端型号
        var isauto=0;//是否自动获取串号信息

        //判断是否自动或手动输入终端串号
        $srhttp.get("!System/Core/Member/~java/MemberConfig.getConfig", {
            business_id: "PUBLIC",
            service_id: "BASE",
            group_id: "SECURITY",
            code: "PARENT_IMEI"
        }, function(data) {
            if(data.body.VALUE == "1") {//自动获取
                //隐藏终端串号选项，显示中端机型选项
                $("#zdch").hide();
                $("#zdjx").show();
                //获取终端机型数据
                $srhttp.get("!ESale/Business/VirtualService/Phone/SimCard/~query/new/Q_TERMINAL_DEVICE",{
                active_type: getActiTypeCode($("#acti_type").text())
                },function(data){
                    console.info("qqq");
                    console.info(data);
        ...
        //$scope.form.group_id="szCreateUserForGroup";
        console.log("========================");console.log($scope.packageForm);
        //发展人参数
        $scope.form = {
                "DEV_CODE": "",
                "DEV_NAME": "",
        ...
        $scope.packageForm.is_white = 1;
        if ($scope.packageForm.numBusId.indexOf('CK') > 0) {
            $scope.packageForm.is_white = 0;
        }
        //console.log($scope.packageForm);
        $scope.ywB = [];
        ...
        $scope.show = {
            ...
            "jitunbianma":false//集团编码
        }
        if ($scope.packageForm.actiVal == "存费送费" || $scope.packageForm.actiVal == "存费送业务"||$scope.packageForm.businessId=="CU.WO4G") {
        ...
        //如果是集客开户就显示
        if ($scope.packageForm.acti_nameId == "CUNFEISONGFEI_JK") {
            $scope.show.jitunbianma = true;
        }
        //往后台写入信息
        ...
        $scope.event = {
            ...
            submit: function() {
               ...
               /* $scope.form.CUSTOMER_NAME = "林创荣";
                ...
                //判断是自动输入串号还是手动输入
                if(isauto==1){//自动输入
                    $scope.TERMINAL_TYPE = $("#myzdjx").val(); //终端型号
                    console.info("终端型号为："+$scope.TERMINAL_TYPE);
                    $srhttp.post("!ESale/Business/VirtualService/Phone/SimCard/~java/TerminalSn.getSn",{
                        nocheck: "true",
                        device: $scope.TERMINAL_TYPE,
                        active_type: getActiTypeCode($("#acti_type").text())
                    },function(data){
                        console.info(11111);
                        $scope.TERMINAL_SN=data.body.sn;
                        $scope.packageForm.numId=data.body.id;
                    ...
                    if( $scope.TERMINAL_SN=="" && $scope.packageForm.policyId_4G!="CUNFEISONGFEI_JK" && $scope.packageForm.policyId!="CU.WO4G"){
                        alert("请输入终端串号");
                        return;
                    }
                    //TODO 自定义产品的提单功能，其实就是移网
                    ...
                    if (data.header.code == 0) {
                        ...
                        $scope.ext = {
                            'ext': {
                                ...
                                'BUSINESS': [{
                                    'key': 'selectProductId',
                                    'val': $scope.packageForm.contractPackage+$scope.packageForm.packageName + $scope.packageForm.contractPlan + $scope.packageForm.firstFee,
                                    'key_as': '产品名称'
                                }, {
                                    'key': 'SIMC_MARK',
                                    ...
                                    'key': 'package_name',
                                    'val': $scope.packageForm.contractPackage+$scope.packageForm.packageName + $scope.packageForm.contractPlan + $scope.packageForm.packagePlan,
                                    'key_as': '套餐名称'
                                }, {
                                    'key': 'package_id',
                                    ...
                                    'key': 'TERMINAL_SN',
                                    'val': $scope.TERMINAL_SN,
                                    'key_as': '终端串号'
                                }, {
                                    'key': 'TERMINAL_MOBILE',
                                    'val': $scope.TERMINAL_TYPE,
                                    'key_as': '终端机型'
                                }, {
                                    'key': 'POLICY_GOOD',
                                    ...
                                    'key': 'NUMBER_FEE',
                                    'val': $scope.packageForm.feeMoney,
                                    'key_as': '预存话费'
                                }],
                                'BASE': [{
                                    'key': 'CUSTOMER_NAME',
                        ...
                        //创建自定义 订单
                        var ppp = "?businessId=" + $scope.packageForm.businessId;
            ...
            creatOrder2: function() {
                ...
                $scope.ext = {
                    'ext': {
                        ...
                        'BUSINESS': [{
                            'key': 'BUSINESS_CODE',
                            'val': $scope.packageForm.acti_nameId=="CUNFEISONGFEI_JK"?"szCreateUserForGroup":$scope.packageForm.businessId,
                            'key_as': '流程编码'
                        },{
                            'key': 'GROUP_ID',
                            'val': $scope.form.group_id,
                ...
                $srhttp.post("!ESale/Mall/Order/~java/Order.create", {
                    payStatus: '-1',
                    businessId: $scope.packageForm.businessId, //TODO
                    list: rdcp.json2str($scope.list),
                    ext: rdcp.json2str($scope.ext),
                    surefee:($scope.packageForm.feeMoney?$scope.packageForm.feeMoney:0)
                }, function(data) {
                    if (data.header.code == 0) {
                        var creatorName = data.body["creator_member_name"];
                        var devId = data.body["member_id"];
