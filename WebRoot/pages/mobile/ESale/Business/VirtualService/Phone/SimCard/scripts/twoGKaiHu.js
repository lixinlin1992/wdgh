/*****选号*/
app.registerCtrl("twoGKaiHuCtrl", ["$scope", "$srhttp", '$filter', '$rootScope',
	function($scope, $srhttp, $filter, $rootScope) {
		...
		//添加下一步跳转的链接
		app.when("/twoGKaiHu_submit", {
			templateUrl: "ESale/Business/VirtualService/Phone/SimCard/pages/twoGKaiHu_submit.html",
			controller: "twoGKaiHuSubmitCtrl"
		});

		$scope.numType = {
			//"value": "ChinaUnicom.Account.4G.BK" //默认白卡
			"value": "ChinaUnicom.Account.2G.BK" //默认白卡
		}
		$scope.d = {
			'BRAND': {
				'key': '中国联通',
		...
		$scope.form = {
			...
			"package_name": $scope.packageName, //!套餐名称,为空不用传值+
			"package_id": $scope.packageID, //!套餐ID,为空不用传值+
		...
		$scope.request = {
			loadNumber: function() {
				...
				$srhttp.get("!ESale/Mall/Goods/~query/Q_GOODS_NUMBER_COMMON", {
					"businessId": $scope.numType.value,
		...
		$scope.selectNumber = function(index) {
			...
			Local.saveStoreJson("numTypeParams", $scope.numType);
			//锁卡
			$scope.lock();
		}
		...
		//查询号码对应的套餐
		$scope.loadnumberPackage = function(id) {

			$srhttp.get("!ESale/Business/VirtualService/Phone/SimCard/~query/portal/Q_2G_PACKAGE_BY_ID", {
					"id": id
				},
				function(data) {
					if (data.header.code == 0) {
						$scope.packageRows = data.body.rows;

						if ($scope.packageRows != '')
							Local.saveStoreJson("packageRows", $scope.packageRows);
					}
				}, {
					//mask: false
				});
