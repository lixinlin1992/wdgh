...
app.registerCtrl("FourGYiTiHuaSubmitCtrl", ["$scope", "$srhttp", "$rootScope",
	function($scope, $srhttp, $rootScope) {
		...
		$scope.submit = function() {
			...
			if ($scope.numType.value == 'ChinaUnicom.Account.4G.CK') {
				...
				/*if (($scope.form.CARDNUMBER) && (imgData.writeCard == '')) {
					alert("成卡卡号格式不正确");
					return;
				}*/
			}
			...
			result['ext'] = {
				'ext': {
					...
					'BUSINESS': [{
						...
						'key': 'BRAND_NAME',
						'val': $scope.d.BRAND.key,
						'key_as': '品牌'
					}, {
						'key': 'BARAND_ID',
						'val': $scope.d.BRAND.value,
						'key_as': '品牌id'
					}, {
			...
			$srhttp.request("!ESale/Mall/Order/~java/Order.create", {
				...
				function(data) {
					if (data.header.code == 0) {
						...
						Local.saveStoreJson("numTypeParams4", "{}");
