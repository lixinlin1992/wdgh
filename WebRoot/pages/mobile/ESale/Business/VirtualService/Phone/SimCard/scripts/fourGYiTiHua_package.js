app.registerCtrl("FourGYiTiHuaPackageCtrl", ["$scope", "$srhttp", "$rootScope",
	function($scope, $srhttp, $rootScope) {
		...
		//添加下一步跳转的链接
		app.when("/fourGYiTiHua_submit", {
			templateUrl: "ESale/Business/VirtualService/Phone/SimCard/pages/fourGYiTiHua_submit.html",
			controller: "FourGYiTiHuaSubmitCtrl"
		});
		...
		$scope.numType = Local.getStoreJson("numTypeParams4");
		$scope.form.businessId = $scope.numType.value;

		//下一步的事件
		$scope.nextPage = function() {
			Local.saveStoreJson("formParams", $scope.form);
			window.location.href = "#/fourGYiTiHua_submit";
		}
	}
]);

/*****4G一体化--套餐类型*/
app.registerCtrl("FourGOpenOrderCtrl", ["$scope", "$srhttp", "$rootScope",
	...
	function($scope, $srhttp, $rootScope) {
		...
		$scope.taocan_select = function(id) {
			...
			//发送广播
			$rootScope.$broadcast('FourGDinnerCtrl', 1);
...
/*****4G一体化套餐--价格选项卡*/
app.registerCtrl("FourGDinnerCtrl", ["$scope", "$srhttp", "$rootScope",
	function($scope, $srhttp, $rootScope) {
		...
		$scope.price_select = function(id) {
			...
			$scope.form.package_id = $(a).attr("package_id"); //套餐id
		...
		//接收广播
		$scope.$on('FourGDinnerCtrl', function(event, data) {
			if (data == 1) {
				$scope.loadMessage();
				$("#fourGPrice").show();
			} else {
