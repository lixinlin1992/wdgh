app.registerCtrl("HandleContractCtrl", ["$scope", "$srhttp", "$rootScope", "$routeParams",
	function($scope, $srhttp, $rootScope, $routeParams) {
		...
		try {
			if ($routeParams.type == '1') {
				...
				//如果type不等于1，说明brand_yiWang一定存在
				brand_yiWang = $scope.interface.brand; //引用，这样就能在rongHe2Ctrl里调用了
				console.log("存在brand_yiWang");
			}
		} catch (e) {
			$scope.interface.brand();
			$rootScope.appTitle = "办合约";
