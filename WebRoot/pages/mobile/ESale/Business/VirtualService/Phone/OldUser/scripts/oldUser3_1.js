...
app.registerCtrl("OldUser3_1Ctrl", ["$scope", "$srhttp", "$rootScope",
	function($scope, $srhttp, $rootScope) {
		...
		$scope.next = function() {
			...
			$scope.form = jQuery.extend($scope.form, $scope.orderList);
...
app.registerCtrl("netLayoutCtrl", ["$scope", "$srhttp", "$rootScope",
	function($scope, $srhttp, $rootScope) {
		//加载开卡网络数据
		$scope.net_true_false = true;

		if ($scope.form.hand == 1) {
			$scope.loadMessage = function() {
					...
					if (data.header.code == 0) {
						...
						setTimeout(function() {
							$scope.$apply(function() {
								if ($scope.form.goodsPackgeId && $scope.form.goodsPackgeId != '') {
									$scope.netLayoutDiv_select();
									var netA = $("#netLayoutDiv a");
									for (var i = 0; i < netA.size(); i++) {
										if ($scope.form.goodsPackgeId.indexOf($("#netLayoutDiv a").eq(i).text().trim()) >= 0) {
											$scope.netLayoutDiv_select(i);
											$scope.net_true_false = false;
											return;
										}
									}
			...
			$scope.loadMessage = function() {
				$srhttp.get("!ESale/Business/VirtualService/Phone/SimCard/~query/portal/Q_3G4G_NETWORK_LIST", {
					...
					if (data.header.code == 0) {
						...
						setTimeout(function() {
							$scope.$apply(function() {
								if ($scope.form.netWork && $scope.form.netWork != 'empty') {
									$scope.netLayoutDiv_select();
									var netA = $("#netLayoutDiv a");
									for (var i = 0; i < netA.size(); i++) {
										if ($scope.form.netWork.indexOf($("#netLayoutDiv a").eq(i).text().trim()) >= 0) {
											$scope.netLayoutDiv_select(i);
											$scope.net_true_false = false;
											return;
										}
	...
	function($scope, $srhttp, $rootScope) {
		...
		$scope.loadMessage = function() {
			if ($scope.form.netWork && $scope.form.netWork != 'empty') {
				...
				$srhttp.get("!ESale/Business/VirtualService/Phone/SimCard/~query/portal/common/Q_PACKAGE_CONFIG_ACCOUNT_TYPE", {
					netWork: $scope.form.goodsPackgeId,
					opentypeFilter: "HY"
				}, function(data) {
					if (data.header.code == 0) {
						$scope.row4 = data.body.rows;
...
app.registerCtrl("ckCtrl", ["$scope", "$srhttp", "$rootScope",
	function($scope, $srhttp, $rootScope) {
		...
		$scope.ckNumConfirm = function() {
			if ($scope.orderList.ckNum == undefined || $scope.orderList.ckNum == "") {
				alert("请输入成卡卡号");
			} else {
				if ($scope.orderList.openAccountType.indexOf("CONTRACT") >= 0) {
					//发送广播
...
app.registerCtrl("contractLastCtrl", ["$scope", "$srhttp", "$rootScope",
	function($scope, $srhttp, $rootScope) {
		$scope.contractLast_select = function(id) {
			...
			console.log($scope.orderList.contractLast);
			$("#nav_bottom").show();
			setTimeout(function() {
				$("#CONTAINER").scrollTop(9999);
			}, 100);
...
app.registerCtrl("packageGroupCtrl", ["$scope", "$srhttp", "$rootScope",
	function($scope, $srhttp, $rootScope) {
		...
		$scope.loadOption = function(index) {
			...
			$srhttp.get("!ESale/Business/VirtualService/Phone/SimCard/~query/portal/common/Q_PACKAGE_CONFIG_PLAN", {
				...
				if (data.header.code == 0) {
					...
					//console.log($scope.groupOption[index].unshift({"FEE": 0, "NAME": "不需要"}));
					index = index + 1;
					//console.log(index);
		...
		$scope.packagegroup_select = function(groupId, id) {
			...
			if ($(a).text() == '不需要') {
				$scope.form.ywB[groupId] = '';
			} else {
				$scope.form.ywB[groupId] = $(a).text();
			}
