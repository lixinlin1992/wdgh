...
//                "rhsjtypeme rhsj,bzqknameio bzqk from jzxt_member where numbere=? and usernameio=?";
        String sql = "select id,title as gh,number as account,username,sex,rxgzsj,"+
                "rhsj,bzqk from jzxt_member where number=? and username=?";
...
//                "from jzxt_article";
        String sql = "select id,title,copyfrom,hits,FROM_UNIXTIME(puttime,'%Y-%c-%d %H:%i:%S') puttime,content " +
                "from jzxt_article";
        if(id != null)
            sql += " where id="+id;
        JSONObject json;
...
//                        notice.setPut_time(rs.getString("put_time"));
                        notice.setCopy_from(rs.getString("copyfrom")==null?"":rs.getString("copyfrom"));
                        notice.setHits(Integer.valueOf(rs.getString("hits")));
                        notice.setPut_time(rs.getString("puttime"));
    ...
    public JSONObject dbjzApply(){
        ...
        String title = request.getParameter("titleory");
        String tel = request.getParameter("telueuid");
        String bzqk = request.getParameter("bzqkstemio");
        String hhzjb = request.getParameter("hhzjbntnal");
        String zfje = request.getParameter("zfjeriptio");
...
//                "values (?,?,?,?,?,?,?,?,?)";
        String sql = "insert into jzxt_dbjz (username,number,title,tel,bzqk,hhzjb,zfje,createtime,status)" +
                "values (?,?,?,?,?,?,?,?,?)";
        JSONObject json;
        try {
            ...
            QueryRunner.update(conn, sql,  new Object[]{username,number ,title,tel ,bzqk, hhzjb,zfje,createtime,status} );
            map.put("header",new Header(0,"success"));
            map.put("body",new HashMap());
            json = (JSONObject) JSON.toJSON(map);
        }
...
//                "from jzxt_dbjz where number =?";
        String sql = "select id ,hhzjb,FROM_UNIXTIME(createtime,'%Y-%c-%d %H:%i:%S') createtime,status "+
                "from jzxt_dbjz where number =?";
...
//                        apply.setTitle(rs.getString("hhzjbntnal"));
                        apply.setTitle(rs.getString("hhzjb"));
    ...
    public JSONObject tkbzApply(){
        ...
        String number = request.getParameter("number");
        String title= request.getParameter("gh");
...
//                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        String sql = "insert into jzxt_tkbz (username,number,title,sex,birthday,zwzc,bysr,jtcy,poxm,poysr,jtzsr,pozwzc,pogzdw,address,tel,bzqk,content,createtime,status)" +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        JSONObject json;
        try {
            QueryRunner.update(conn, sql,  new Object[]{name,number,title,sex,birthday,zwzc,bysr,jtcy,poxm,poysr,jtzsr,pozwzc,pogzdw,address,tel,bzqk,content,createtime,status} );
...
//            "from jzxt_tkbz where numbergent =? ";
        String sql = "select content, FROM_UNIXTIME(createtime,'%Y-%c-%d %H:%i:%S') createtime,status "+
                "from jzxt_tkbz where number =? ";
        System.out.println("!!! "+ number);
        String status = request.getParameter("status");
        if(status != null)
            ...
            ArrayList applies = (ArrayList)QueryRunner.queryResultSet(conn, sql,new Object[]{number}, new ResultSetHandler<Object>() {
                ...
                public Object handle(ResultSet rs) throws Exception {
                    ...
                    while (rs.next()) {
                        ...
                        apply.setTitle(rs.getString("content"));
                        int status = Integer.valueOf(rs.getString("status")).intValue();
                        if (status <= 10 && status >= 0)
                            apply.setStatus(Status[status]);
                        else apply.setStatus(Status[0]);
