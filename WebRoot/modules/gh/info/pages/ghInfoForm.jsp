    ...
    <div align="center">
        ...
        <div class="SR_searchTableBox">
            <div class="barquerycontent">
                <form id="ghInfoForm" name="ghInfoForm">
...
<script type="text/javascript">
    ...
    rdcp.ready(function () {
        ...
        getParamsByPaCode("type", "BI_GH_INFO", "TYPE", function () {
            if (option == "add") {
            }
            else if (option == "edit") {
                //如果option为edit，则加载表单
                rdcp.form.load("ghInfoForm", "!gh/info/~query/Q_GET_GH_INFO", 'info_id=' + info_id, function (data) {
    ...
    function sureBtn() {
        ...
        $("#content").val(content);
        if (title == "" || title == null) {
            $.messager.alert('提示', '请输入工会概况标题！', 'info');
            return false;
        }
        if (type == "" || type == null){
        ...
        if (content == "" || content == null) {
            $.messager.alert('提示', '请编辑工会概况内容！', 'info');
            return false;
        }
        if (option == "add") {
            rdcp.form.submit("ghInfoForm", {
                url: "!gh/info/~query/Q_ADD_GH_INFO",
                success: function (data) {
                    $.messager.alert('提示', '工会概况信息发布成功！', 'info',function () {
                        cancel();
                    });
                }
            }, {"mask": true});
            ...
            $("#info_id").val(info_id);
            rdcp.form.submit("ghInfoForm", {
                url: "!gh/info/~query/Q_UPDATE_GH_INFO" ,
                success: function (data) {
                    $.messager.alert('提示', '工会概况信息修改成功！', 'info',function () {
                        cancel();
                    });
                }
            }, {"mask": true});
    ...
    {
        if (option == "add") {
            CloseTab("addGhInfo", "发布信息");
        }
        else if (option == "edit"){
            CloseTab("editGhInfo", "修改信息");
        }
