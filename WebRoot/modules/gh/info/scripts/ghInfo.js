        ...
        [
            {field: 'ID', title: 'ID', hidden: true, sortable: false, align: 'center', width: 0},
            {
                ...
                formatter: function (cell, row, index) {
                    var edit = '<a class="btn_edit" href="javascript:void(0);"  onclick="editGhInfo(\'' + row.ID + '\');">修改</a>';
                    var del = '<a class="btn_delete" href="javascript:void(0);"  onclick="delGhInfo(\'' + row.ID + '\');">删除</a>';
                    return  edit+ "&nbsp;" + del;
                }
            },
...
rdcp.ready(function () {
    ...
    rdcp.grid('listdt', '!gh/info/~query/Q_GH_INFO_LIST', "searchForm", params);
...
function addGhInfo(){
    ...
    var title = "发布信息";
    //标签页url
    var url = "!gh/info/~/pages/ghInfoForm.jsp?option=add";
    OpenTab(tabId, title, url);
}
//修改工会概况
function editGhInfo(info_id){
    //标签页ID
    var tabId = "editGhInfo";
    //标签页TILE
    var title = "修改信息";
    //标签页url
    var url = "!gh/info/~/pages/ghInfoForm.jsp?option=edit&info_id="+info_id;
    OpenTab(tabId, title, url);
}
//删除工会概况
function delGhInfo(info_id){
    $.messager.confirm("警告", "您确定要删除该条数据吗？",
        function (yesBtn) {
            if(yesBtn) {
                rdcp.request("!gh/info/~query/Q_DEL_GH_INFO", "info_id="+info_id, function(data){
