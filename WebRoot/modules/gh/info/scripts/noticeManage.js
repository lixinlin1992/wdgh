        ...
        [
            {field: 'ID', title: 'ID', hidden: true, sortable: false, align: 'center', width: 0},
            {
                ...
                formatter: function (cell, row, index) {
                    var edit = '<a class="btn_edit" href="javascript:void(0);"  onclick="editNotice(\'' + row.ID + '\');">修改</a>';
...
function addNotice(){
    ...
    var title = "发布通知公告";
    //标签页url
    var url = "!gh/info/~/pages/noticeForm.jsp?option=add";
...
function editNotice(id){
    //标签页ID
    var tabId = "editNotice";
    //标签页TILE
    var title = "修改通知公告";
    //标签页url
    var url = "!gh/info/~/pages/noticeForm.jsp?option=edit&id="+id;
    OpenTab(tabId, title, url);
