...
//				var p = $('<div class="tree-node-proxy tree-dnd-no"></div>').appendTo('body');
			...
			onStopDrag: function(){
				...
				if (node.id == 'easyui_tree_node_id_temp'){
					node.id = '';
					updateNode(target, node);
				}
...
//			setTimeout(function(){
//				var val = $(target).val();
//				if (val != opts.parser.call(target, val)){
//					setValue(target, val);
...
//			input.trigger('mouseleave');
	...
	$.fn.combo.parseOptions = function(target){
		var t = $(target);
		return $.extend({}, $.fn.validatebox.parseOptions(target), $.parser.parseOptions(target, [
			'width','height','separator',{panelWidth:'number',editable:'boolean',hasDownArrow:'boolean',delay:'number'}
		]), {
